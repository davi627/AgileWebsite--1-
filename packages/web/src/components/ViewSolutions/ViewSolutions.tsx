...
const ViewSolutions: React.FC = () => {
  ...
  const handleDelete = async (solutionId: string) => {
    const action = async () => {
      try {
        const response = await axios.delete(`${API_BASE_URL}/solns/solution/${solutionId}`);
      ...
      <div className="space-y-4">
        {solutions.length === 0 ? (
          ...
          solutions.map((solution) => (
            <div key={solution._id} className="flex justify-between items-center p-4 border border-gray-300 rounded-md">
